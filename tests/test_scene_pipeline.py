...
from test_court_calibration import KNOWN_COURT_TO_IMAGE
...
class ScenePipelineTest(unittest.TestCase):
    def test_five_frame_checkpoints_hold_expire_and_recover(self):
        ...
        calibrator = CourtCalibrator(CourtHomographyEstimator(
    ...
    def test_default_runs_court_detection_every_frame(self):
        court = FakeCourtDetector({index: synthetic_detection(index) for index in range(3)})
        ...
        self.assertTrue(all(result.calibration.source is CalibrationSource.DETECTED
